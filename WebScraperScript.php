...
function data_update_check($ws){
	...
	return needTOScrape($lastValue,$date_test);

}

function needToScrape($fileDateStr, $currentDateStr) {//$fileDateStr, $currentDateStr are strings, function turns into date object
    // Parse file date and current date
    $fileDate = DateTime::createFromFormat('j M y', $fileDateStr);
    $currentDate = DateTime::createFromFormat('j M y', $currentDateStr);

    // Check if file date is not 7th of current month and current date is 7th and less than 21st of current month
    if ($fileDate->format('j') != 7 && $currentDate->format('j') == 7 && $currentDate->format('j') < 21) {
        return true;
    }
    // Check if file date is 7th of current month and current date is less than 21st of current month
    elseif ($fileDate->format('j') == 7 && $currentDate->format('j') < 21) {
        return false;
    }
    // Check if file date is 7th of current month and current date is 21st of current month and less than 7th of next month
    elseif ($fileDate->format('j') == 7 && $currentDate->format('j') == 21) {
        $nextMonth = clone $currentDate;
        $nextMonth->add(new DateInterval('P1M'));
        $nextMonth->setDate($nextMonth->format('Y'), $nextMonth->format('m'), 7);
    ...
    // Check if file date is 21st of current month and current date is 21st of current month and less than 7th of next month
    elseif ($fileDate->format('j') == 21 && $currentDate->format('j') == 21) {
        $nextMonth = clone $currentDate;
        $nextMonth->add(new DateInterval('P1M'));
        $nextMonth->setDate($nextMonth->format('Y'), $nextMonth->format('m'), 7);
...
$file_titles = array("Lexus_recon","Honda_recon","Toyota_recon","Mercedes_recon");
...
for($counttest = 0;$counttest < count($file_titles);$counttest++){
	...
	for($scount = 0; $scount < count($Modelname); $scount++){
		...
		if(data_update_check($worksheet)){
